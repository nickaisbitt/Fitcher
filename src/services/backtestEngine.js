...
class BacktestEngine {
  ...
  calculateExecutionPrice(side, targetPrice, marketData) {
    ...
    if (this.config.slippageModel === 'dynamic' && marketData.recentCandles?.length >= 2) {
      ...
      const n = candles.length - startIdx;
    ...
    for (let i = 0; i < this.trades.length; i++) {
      ...
      if (trade.side === 'buy') {
        ...
        while (remainingSellAmount > 0 && openBuysHead < openBuys.length) {
          const buyRecord = openBuys[openBuysHead];
          const buy = buyRecord.trade;
          const matchAmount = Math.min(remainingSellAmount, buyRecord.remainingAmount);
          ...
          buyRecord.remainingAmount -= matchAmount;

          if (buyRecord.remainingAmount <= 0) {
            openBuysHead++;
          }
        }
      }
