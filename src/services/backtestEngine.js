...
class BacktestEngine {
  ...
  calculateTradeStats() {
    ...
    // Use an array with an index pointer to avoid O(N^2) unshift/shift operations
    const openBuys = [];
    let openBuyIndex = 0;

    // Fast-path trade collection
    for (let i = 0; i < this.trades.length; i++) {
      const trade = this.trades[i];
      if (trade.side === 'buy') {
        // Avoid object spread overhead; manually construct what we need
        openBuys.push({
          price: trade.price,
    ...
    // Compute stats in a single pass instead of multiple filter/reduce passes
    let winningTrades = 0;
    let losingTrades = 0;
    let totalWin = 0;
    let totalLoss = 0;
