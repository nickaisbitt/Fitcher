...
class BacktestEngine {
  ...
  calculateExecutionPrice(side, targetPrice, marketData) {
    ...
    if (this.config.slippageModel === 'dynamic' && marketData.recentCandles?.length >= 2) {
      ...
      // ⚡ Bolt Optimization: Welford's online algorithm for computing variance
      // Avoids intermediate array allocations and multiple `.reduce()` calls.
      // Impact: Single O(1) space pass instead of allocating intermediate arrays.
      let count = 0;
      let mean = 0;
      let M2 = 0;
  ...
  calculateTradeStats() {
    ...
    // ⚡ Bolt Optimization: Replace O(N^2) Array.shift() and object spreading with O(N) index pointer and direct assignment.
    // Impact: Drastically reduces GC overhead and algorithmic complexity during large backtests.
    const openBuys = [];
    let openBuyIndex = 0;
    ...
    // ⚡ Bolt Optimization: Calculate stats in a single pass instead of multiple `.filter()` and `.reduce()` chains.
    // Impact: Avoids multiple intermediate array allocations and redundant iteration.
