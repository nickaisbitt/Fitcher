...
class BacktestEngine {
  ...
  calculateTradeStats() {
    ...
    for (const trade of this.trades) {
      if (trade.side === 'buy') {
        // Assign specific properties instead of object spread to avoid GC overhead
        openBuys.push({
          price: trade.price,
        ...
        while (remainingSellAmount > 0 && openBuysIndex < openBuys.length) {
          ...
          if (buy.remainingAmount <= 0) {
            openBuysIndex++; // Use index pointer instead of Array.shift()
          }
        }
      }
    }
