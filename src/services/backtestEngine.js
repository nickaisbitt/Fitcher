...
class BacktestEngine {
  ...
  calculateExecutionPrice(side, targetPrice, marketData) {
    ...
    if (this.config.slippageModel === 'dynamic' && marketData.recentCandles?.length >= 2) {
      ...
      // Bolt Optimization: Welford's online algorithm to calculate mean and variance in a single O(N) pass
      let count = 0;
      let mean = 0;
      let M2 = 0;
  ...
  calculateTradeStats() {
    ...
    // Bolt Optimization: Use buyIdx pointer instead of openBuys.shift() to eliminate O(N²) array re-indexing
    let buyIdx = 0;

    for (const trade of this.trades) {
      if (trade.side === 'buy') {
        // Bolt Optimization: Explicit assignment instead of object spreading ({ ...trade }) in hot loop
        openBuys.push({
          price: trade.price,
    ...
    // Bolt Optimization: Compute totals in a single pass instead of chaining .filter() and .reduce()
    let winningTrades = 0;
    let losingTrades = 0;
    let totalWin = 0;
    let totalLoss = 0;
  ...
  calculateSharpeRatio() {
    ...
    // Bolt Optimization: Welford's online algorithm to compute mean and variance in a single O(N) pass
    let count = 0;
    let mean = 0;
    let M2 = 0;
