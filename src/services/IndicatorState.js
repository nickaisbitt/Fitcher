...
class IndicatorState {
  constructor() {
    ...
    // Bollinger state: { buffer, head, count, sum, sumSq }
    this.bb = { buffer: new Float64Array(20), head: 0, count: 0, sum: 0, sumSq: 0, period: 20, stdDev: 2, value: null };
  ...
  _updateSMA(name, close, period) {
    if (!this.smas[name]) {
      this.smas[name] = { buffer: new Float64Array(period), head: 0, count: 0, sum: 0, value: null };
    }
    const state = this.smas[name];

    if (state.count === period) {
      state.sum -= state.buffer[state.head];
    } else {
      state.count++;
    }
