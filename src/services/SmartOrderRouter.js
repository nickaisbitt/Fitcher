...
class SmartOrderRouter {
  ...
  async calculateVolatility(pair) {
    ...
    try {
      ...
      let sum = 0;
      for (let i = 0; i < len; i++) sum += returns[i];
      const mean = sum / len;

      let sumSq = 0;
      for (let i = 0; i < len; i++) sumSq += Math.pow(returns[i] - mean, 2);
  ...
  async assessLiquidity(pair) {
    ...
    try {
      ...
      const bidsLen = orderBook.bids.length;
      for (let i = 0; i < bidsLen; i++) totalBidVolume += orderBook.bids[i][1];

      let totalAskVolume = 0;
      const asksLen = orderBook.asks.length;
      for (let i = 0; i < asksLen; i++) totalAskVolume += orderBook.asks[i][1];
