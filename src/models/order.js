...
class Order {
  ...
  addTrade(trade) {
    ...
    // Recalculate filled amount and average price using incremental state (O(1))
    const parsedAmount = parseFloat(trade.amount);
    const parsedPrice = parseFloat(trade.price);
    const parsedFee = parseFloat(trade.fee) || 0;

    if (this._totalValue === undefined) {
        this._totalValue = this.filledAmount * (this.averagePrice || 0);
    }

    this.filledAmount += parsedAmount;
    this.remainingAmount = this.amount - this.filledAmount;
    this._totalValue += (parsedPrice * parsedAmount);
  ...
  static fromJSON(json) {
    ...
    order._totalValue = (json.filledAmount || 0) * (json.averagePrice || 0);
    order.fee = json.fee;
