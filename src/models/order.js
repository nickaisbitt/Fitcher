...
class Order {
  ...
  addTrade(trade) {
    ...
    const currentAmount = parseFloat(trade.amount);
    const currentPrice = parseFloat(trade.price);
    const currentFee = parseFloat(trade.fee) || 0;

    this.filledAmount += currentAmount;
    this.remainingAmount -= currentAmount;
    this._totalValue += (currentPrice * currentAmount);
    this.fee += currentFee;
